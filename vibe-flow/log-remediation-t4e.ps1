...
$driftPayload = @{
    ...
    spec_diff       = '{"task":"T4e","finding":"W3-002 (drift reports + ssd evaluations)","status":"resolved","impl":["SaveDriftReport tags project_id","LatestDriftForArtifact filters by project_id","ListDriftReports filters by project_id","SaveSDDEvaluation tags project_id","LatestSDDEvaluation filters by project_id","ListSDDEvaluations filters by project_id"],"tests":["TestProject_DriftReports_CrossProject_Isolated PASS","TestProject_SDDEvaluations_CrossProject_Isolated PASS"]}'
    judge_reasoning = "T4e closed: W3-002 for drift reports and SSD evaluations resolved. 6 methods now requireProject + filter by project_id. Drift test creates a real artifact first (FK on vibe_drift_reports.artifact_id). 46/46 green suite. Spec 171 still drift_detected until T4f-T4g + T5."
...
Write-Host ""
Write-Host "=== T4e (Drift + SSD) closed: drift_resolved ==="