...
//	  5. mcp.NewServer → register all canonical tools
//	  6. stdio transport
...
// (register the canonical tools) is the caller's responsibility
// because the registry is populated by the per-namespace tool files
// at package init time (or via explicit Register* calls in tests).
// Step 6 is
