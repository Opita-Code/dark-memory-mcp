...
// 25 tools into the Registry. Called from internal/server/server.go's
...
// RegisterAll wires all 25 dark_memory_* tools into the registry, in
// the canonical order (spec 164, bridge.4). Safe to call once per
// Registry; subsequent calls are no-ops if the tools are already
// registered.
//
...
// The canonical 25-tool surface is the union of all namespaces.
func RegisterAll(reg *Registry, orch *orchestration.Orchestrator, st store.Store) error {
	...
	// Sanity check: registry must contain all 25 canonical tools
	...
	if got := len(reg.ListCanonical()); got != 25 {
		return fmt.Errorf("tools: RegisterAll: expected 25 tools, got %d", got)
	}
	return nil
}